...
<form class="form-content row g-3" action="{{ route('physician.registration.store')  }}" method="POST"
    ...
    <script>
        ...
        addQualificationBtn.addEventListener('click', function () {
            ...
            const newQualificationSection = document.createElement('div');
            newQualificationSection.classList.add('col-md-12');

            @php
                $degreeLevel = old('degree_level');
                $countryValue = old('country');
            @endphp

            newQualificationSection.innerHTML = `
    <hr class="hr hr-blurry"/>
...
<div class="col-md-3 form-floating">
    <select id="degree_level" name="degree_level" class="form-select {{ $errors->has('degree_level') ? 'is-invalid' : '' }}" required>
        <option value="">Select degree</option>
        <option value="Dr">Diploma</option>
        <option value="Prof">Associate Degree</option>
        <option value="MD">Bachelor's Degree</option>
    ...
    @if ($errors->has('degree_level'))
            <div class="invalid-feedback">{{ $errors->first('degree_level') }}</div>
    @endif
            </div>
...
`;
            ...
            qualificationsContainer.appendChild(newQualificationSection);
        });
    </script>
